...
export default class DriftApp extends React.Component {
  ...
  renderNav() {
    return <Nav onPrevious={this.handleClickPrevious.bind(this)} hasPrevious={this.state.showIndex > 0}
                onNext={this.handleClickNext.bind(this)} hasNext={this.state.showIndex < this.state.numSlides - 1} />
  }
  render() {
    return (
      <Frame>
        <Style rules={styles} />
        <Carousel showIndex={this.state.showIndex} nav={this.renderNav()} width={configStyles.imageWidth} height={configStyles.imageHeight}>
